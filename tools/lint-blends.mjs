...
function resolve(raw) {
  ...
  if (explicitCas && DB[explicitCas])
    return { kind: 'material', cas: explicitCas, name: DB[explicitCas].name, via: 'explicit_cas' };
  const lk = label.toLowerCase();
  for (const e of db)
    if (e.name.toLowerCase() === lk)
      return { kind: 'material', cas: e.cas, name: e.name, via: 'canonical' };
  if (NAME_TO_CAS[lk])
    return {
      kind: 'material',
      cas: NAME_TO_CAS[lk],
      name: DB[NAME_TO_CAS[lk]].name,
      via: 'synonym',
    };
  if (trades[lk] && DB[trades[lk]])
    return { kind: 'material', cas: trades[lk], name: DB[trades[lk]].name, via: 'trade' };
  if (BLEND_SHORTHAND_TO_CAS[lk] && DB[BLEND_SHORTHAND_TO_CAS[lk]]) {
    return {
      kind: 'material',
      cas: BLEND_SHORTHAND_TO_CAS[lk],
      name: DB[BLEND_SHORTHAND_TO_CAS[lk]].name,
      via: 'shorthand',
    };
  }
...
const _NAME_SECONDARY_AMINE =
  /\b(dimethyl[- ]?anthranilate|n-methyl[- ]?anthranilate|indole|skatole|quinoline)\b/;
const _NAME_PRIMARY_AMINE = /\banthranilate\b/;
const _NAME_THIOL = /\b(mercapto|thiol|mercaptan|thio[- ]?acetate|sulfanyl)\b/;
const _NAME_ALDEHYDE =
  /(aldehyde|cinnamaldehyde|citral|citronellal|heliotropin|vanillin|helional|hexenal|nonanal|octanal|decanal|undecanal|dodecanal|tridecanal|lauric\s+aldehyde)/i;
const _NAME_PHENOL =
  /\b(eugenol|isoeugenol|thymol|guaiacol|carvacrol|chavicol|cresol|vanillin|methyl\s+salicylate|eugen|phenol)\b/;
...
for (const e of db) {
  ...
  for (const raw of blends) {
    ...
    if (
      r.kind === 'material' &&
      r.via === 'shorthand' &&
      rawLabel.trim().toLowerCase() !== r.name.toLowerCase()
    ) {
      findings.shorthandDrift.push({
        cas: e.cas,
        name: from,
        label: rawLabel,
        canonical: r.name,
    ...
    if (r.kind === 'material') {
      ...
      if (partnerEntry && isProhibited(partnerEntry)) {
        findings.prohibitedPartner.push({
          cas: e.cas,
          name: from,
          label: rawLabel,
          targetCas: r.cas,
      ...
      if (sourceGroups.size) {
        ...
        for (const [gA, gB, reaction] of INCOMPATIBLE_PAIRS_HIGH) {
          if (
            (sourceGroups.has(gA) && targetGroups.has(gB)) ||
            (sourceGroups.has(gB) && targetGroups.has(gA))
          ) {
            findings.incompatiblePair.push({
              cas: e.cas,
              name: from,
              label: rawLabel,
              targetCas: r.cas,
    ...
    if (r.kind === 'material') {
      ...
      const partnerBlends = (partner.blends_with || []).map(x =>
        x && typeof x === 'object' ? String(x.label || '').toLowerCase() : String(x).toLowerCase()
      );
      ...
      if (!hit)
        findings.reciprocity.push({ from, fromCas: e.cas, to: r.name, toCas: r.cas, via: r.via });
    }
  }
}

// ── Aggregate stats
const totalLinks = db.reduce(
  (n, e) => n + (Array.isArray(e.blends_with) ? e.blends_with.length : 0),
  0
);
...
for (const u of findings.unresolved)
  unresolvedFreq.set(u.label.toLowerCase(), (unresolvedFreq.get(u.label.toLowerCase()) || 0) + 1);
...
function hr(char = '─', n = 68) {
  return char.repeat(n);
}
console.log(hr('═'));
console.log(
  ' Blends-with lint report — ' + db.length + ' materials, ' + totalLinks + ' link slots'
);
console.log(hr('═'));
...
if (findings.selfref.length) {
  for (const s of findings.selfref.slice(0, 10))
    console.log('  ' + s.cas.padEnd(12) + s.name + '  ← lists itself as "' + s.label + '"');
...
if (findings.reciprocity.length) {
  ...
  for (const r of sample)
    console.log('    ' + r.from + '  →  ' + r.to + '    (' + r.to + ' missing ' + r.from + ')');
  if (findings.reciprocity.length > 15)
    console.log('    … +' + (findings.reciprocity.length - 15) + ' more');
}
...
if (findings.shorthandDrift.length) {
  ...
  for (const r of sample)
    console.log(
      '    ' + r.name + ':  "' + r.label + '"  →  "' + r.canonical + '"  (' + r.targetCas + ')'
    );
  if (findings.shorthandDrift.length > 15)
    console.log('    … +' + (findings.shorthandDrift.length - 15) + ' more');
}
...
if (findings.duplicateCas.length) {
  ...
  if (findings.duplicateCas.length > 10)
    console.log('    … +' + (findings.duplicateCas.length - 10) + ' more');
}
...
if (findings.prohibitedPartner.length) {
  ...
  for (const r of sample)
    console.log('    ' + r.name + '  →  ' + r.targetName + '  (' + r.targetCas + ')');
  if (findings.prohibitedPartner.length > 10)
    console.log('    … +' + (findings.prohibitedPartner.length - 10) + ' more');
}

console.log('\n' + hr());
console.log(
  ' (i) Chemically-incompatible partner (HIGH reactive): ' + findings.incompatiblePair.length
);
console.log(hr());
if (findings.incompatiblePair.length) {
  ...
  for (const r of sample)
    console.log('    ' + r.name + '  →  ' + r.targetName + '   (' + r.reaction + ')');
  if (findings.incompatiblePair.length > 10)
    console.log('    … +' + (findings.incompatiblePair.length - 10) + ' more');
}
...
if (findings.nonPerfumery.length) {
  ...
  for (const r of sample)
    console.log('    ' + r.name + '  →  ' + r.targetName + '  (' + r.targetCas + ')');
  if (findings.nonPerfumery.length > 10)
    console.log('    … +' + (findings.nonPerfumery.length - 10) + ' more');
}
...
if (findings.inconsistentCas.length) {
  ...
  if (findings.inconsistentCas.length > 10)
    console.log('    … +' + (findings.inconsistentCas.length - 10) + ' more');
}
...
console.log(
  '   selfref=' +
    findings.selfref.length +
    '  unresolved=' +
    findings.unresolved.length +
    '  reciprocity=' +
    findings.reciprocity.length +
    '  shorthandDrift=' +
    findings.shorthandDrift.length
);
console.log(
  '   duplicateCas=' +
    findings.duplicateCas.length +
    '  prohibitedPartner=' +
    findings.prohibitedPartner.length +
    '  incompatiblePair=' +
    findings.incompatiblePair.length +
    '  nonPerfumery=' +
    findings.nonPerfumery.length +
    '  inconsistentCas=' +
