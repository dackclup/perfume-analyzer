"""
app.py  v5.0 — Shows ALL PubChem data sections
...
with st.sidebar:
    ...
    st.divider()
    st.caption("v5.0 · Full PubChem extraction")
...
for i in range(len(st.session_state.inputs)):
    ...
    with cols[0]:
        v = st.text_input(f"Material {i+1}", value=st.session_state.inputs[i],
            key=f"inp_{i}", placeholder="e.g. Linalool, Iso E Super, Hedione …",
    ...
    for mat in st.session_state.results:
        ...
        with st.expander(f"✅  {mat.name}", expanded=True):
            ...
            with c:
                ...
                if mat.tenacity: st.markdown(f"**Tenacity:** {mat.tenacity}")
                if mat.tenacity_hours: st.markdown(f"**Duration:** {mat.tenacity_hours}")
                if not mat.tenacity: st.caption("No tenacity data.")
            ...
            if any([mat.ifra_guidelines, mat.usage_levels, mat.blends_well_with]):
                ...
                with s1:
                    ...
                    if mat.ifra_guidelines: st.markdown(f"**IFRA:** {mat.ifra_guidelines}")
                    if mat.usage_levels: st.markdown(f"**Usage:** {mat.usage_levels}")
                with s2:
...
if st.session_state.results and st.session_state.done:
    ...
    with dl1:
        ...
        st.download_button("📄 สำหรับคนอ่าน (.pdf)", data=pdf_bytes,
            ...
            use_container_width=True)
        st.caption("**PDF** — จัดหน้าสวย ปริ้น A4 ได้เลย พร้อมข้อมูล PubChem ทุก section")
    with dl2:
        ...
        st.download_button("🤖 สำหรับ AI อ่าน (.json)", data=ai_json,
            ...
            use_container_width=True)
        st.caption("**JSON** — ข้อมูลครบทุก section จาก PubChem + perfumery DB")
