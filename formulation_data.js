...
const PEROXIDE_CONTROL = {
  ...
  "123-35-3":   { maxPV: 20, unit: "mmol/L", name: "beta-Myrcene" },
  "586-62-9":   { maxPV: 20, unit: "mmol/L", name: "alpha-Terpinolene" },
};
...
const MATERIAL_PROPERTIES = {
  // ── Orphan-fallback table (was 127 entries before v207) ──
  //
  // density + boiling_point now live in perfumery_data.js entries
  // (added for the 91 CAS that have a curated DB entry, see v207).
  // What remains here are CAS that are NOT YET in the curated DB but
  // are referenced by other engineering tables (ANTOINE_COEFFICIENTS,
  // SMILES_FALLBACK, ODOR_THRESHOLDS, AROMACHOLOGY_SCORES, etc.).
  // formulation_engine.js reads DB[cas] first; this table is the
  // fallback when a material is added to a formulation but has no
  // curated DB entry. Long-term goal: each orphan CAS becomes a real
  // perfumery_data.js entry and this table shrinks to zero.
  "13877-91-3":   { mw: 136.23, density: 0.8, logP: 4.17, bp: 177, smiles: "CC(=CCC=C(C)C)C=C" },
  ...
  "628-63-7":     { mw: 130.18, density: 0.876, logP: 2.3, bp: 149, smiles: "CCCCCOC(C)=O" },
  ...
  "150-84-5":     { mw: 198.3, density: 0.89, logP: 4.04, bp: 244, smiles: "CC(CCCC(C)OC(C)=O)CC=C" },
  "103-45-7":     { mw: 164.2, density: 1.03, logP: 2.3, bp: 232, smiles: "CC(=O)OCCc1ccccc1" },
  ...
  "89-78-1":      { mw: 156.27, density: 0.89, logP: 3.38, bp: 212, smiles: "CC1CCC(C(C1)O)C(C)C" },
  "89-80-5":      { mw: 154.25, density: 0.895, logP: 2.96, bp: 207, smiles: "CC1CCC(C(=O)C1)C(C)C" },
  "99-49-0":      { mw: 150.22, density: 0.96, logP: 2.74, bp: 231, smiles: "CC1=CC(=O)C(CC1)C(=C)C" },
  "104-46-1":     { mw: 148.2, density: 0.988, logP: 3.39, bp: 235, smiles: "COc1ccc(/C=C/C)cc1" },
  "100-51-6":     { mw: 108.14, density: 1.045, logP: 1.1, bp: 205, smiles: "OCc1ccccc1" },
  "14901-07-6":   { mw: 192.3, density: 0.946, logP: 4.42, bp: 243, smiles: "CC1=CC(=O)C(C)(C)C=C1/C=C/C(C)C" },
  "127-42-4":     { mw: 206.32, density: 0.933, logP: 4.3, bp: 237, smiles: null },
  ...
  "18127-01-0":   { mw: 204.31, density: 0.953, logP: 3.8, bp: 293, smiles: null },
  ...
  "23726-91-2":   { mw: 192.3, density: 0.934, logP: 4.2, bp: 260, smiles: null },
  "23696-85-7":   { mw: 190.28, density: 0.952, logP: 3.6, bp: 274, smiles: null },
  "106-44-5":     { mw: 108.14, density: 1.034, logP: 1.94, bp: 202, smiles: "Cc1ccc(O)cc1" },
  "3738-00-9":    { mw: 236.4, density: 1, logP: 4.5, bp: 290, smiles: null },
  "21145-77-7":   { mw: 258.4, density: 1.063, logP: 5.7, bp: 300, smiles: null },
  "81-14-1":      { mw: 294.3, density: 1.16, logP: 3.87, bp: 350, smiles: null },
  ...
  "198404-98-7":  { mw: 210.31, density: 0.978, logP: 3.4, bp: 285, smiles: null },
};
...
const FRAGRANCE_WHEEL = {
  // Edwards' 2021 layout with the three "pure" transitional slices centred
  // EXACTLY on the cardinal dividers. Each main-family inner band spans
  // 90° (at SVG 180°/270°/0°/90° boundaries), and the three transitional
  // slices straddle those boundaries — half of each sits in one main
  // family, half in the adjacent one. Non-transitional slices fit into
  // the remaining arc of each band, so slice widths vary:
  //
  //   Fruity        centred at 180° (9 o'clock, Fresh↔Floral)   — 25.71°
  //   Aromatic      centred at  90° (6 o'clock, Woody↔Fresh)    — 25.71°
  //   Woody Amber   centred at 360° (3 o'clock, Amber↔Woody)    — 25.71°
  //   Floral Amber  at 257.14° (just left of 12; Floral↔Amber)  — 25.71°
  //     (12 o'clock stays a hard boundary — Floral Amber sits inside
  //      FLORAL, Soft Amber inside AMBER, no straddling slice there.)
  //
  //   Inside FRESH  (90°–180°, minus half each of Aromatic + Fruity):
  //     Citrus, Water, Green at 21.43° each
  //   Inside FLORAL (180°–270°, minus half of Fruity):
  //     Floral, Soft Floral, Floral Amber at 25.71° each
  //   Inside AMBER  (270°–360°, minus half of Woody Amber):
  //     Soft Amber, Amber at 38.57° each
  //   Inside WOODY  (0°–90°, minus half each of Woody Amber + Aromatic):
  //     Woods, Mossy Woods, Dry Woods at 21.43° each
  segments: [
    // Each slice carries a SOLID colour. The five transitional subfamilies
    // (Aromatic, Fruity, Floral Amber, Woody Amber, Woods) keep the
    // `transitional: true` flag for italic labels + tooltip but no longer
    // two-tone gradient fills — Edwards' printed wheel paints them as single
  ...
  // The renderer used to derive these from the segment angles of each
  // mainGroup, but with three transitional slices straddling the cardinal
  // boundaries (Fruity at 9 o'clock, Aromatic at 6, Woody Amber at 3) a
  // segment-derived arc would extend into the adjacent band by a half-
  // slice width. Pinning the arcs to the cardinals keeps each inner
  // "NOTES" band exactly 90° wide and touching its neighbours.
  mainBands: {
    fresh:  [90, 180],
    floral: [180, 270],
    amber:  [270, 360],
  ...
  // Used by generateFromBrief's family-match scoring (to give partial credit
  // to both adjacent primaries) and by the SVG renderer to locate gradient
  // stops. Edwards positions FIVE slices as transitional — three that sit
  // on a cardinal boundary (Fruity at 9, Aromatic at 6, Woods at 3) plus
  // two that sit inside an anchor band but carry a blended colour
  // (Floral Amber inside FLORAL, Woody Amber inside AMBER).
  transitional: {
    fruity:           ['citrus',     'floral'],
    floral_amber:     ['floral',     'amber'],
    woody_amber:      ['amber',      'mossy_woods'],
    woods:            ['amber',      'mossy_woods'],
